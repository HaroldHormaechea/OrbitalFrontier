 ...
 * the player's credits and cargo after the action, and whether anything event-worthy changed.
 *
 ...
 * [credits] and [cargo] instances returned unchanged — so the caller (the play screen on device, the
 * simulation on the JVM) can cheaply detect "nothing happened" with a reference check and skip the
 * autosave. The caller folds the values back into its state.
 *
